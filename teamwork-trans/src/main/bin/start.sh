...
if [ "$1" = "jmx_local" ]; then
    JAVA_JMX_OPTS=" -Djava.rmi.server.hostname=0.0.0.0 -Dcom.sun.management.jmxremote.port=1099 -Dcom.sun.management.jmxremote.ssl=false -Dcom.sun.management.jmxremote.authenticate=false "
fi
...
if [ -n "$BITS" ]; then
    ...
    JAVA_MEM_OPTS=" -server -Xms2048m -Xmx2048m -XX:PermSize=128m -XX:SurvivorRatio=2 -XX:+UseParallelGC "
fi
