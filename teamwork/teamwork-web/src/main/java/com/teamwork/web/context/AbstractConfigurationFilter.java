...
public abstract class AbstractConfigurationFilter implements Filter {
     ...
     /** 静态文件后缀名 */
     private Set<String> ignoreSuffix = new HashSet<>();

     private static final String[] DEFAULT_STATIC_FILE_SUFFIX = new String[]{"jpg", "jpeg","ico", "txt", "doc", "ppt", "xls", "pdf", "gif", "png", "bmp", "css", "js","swf", "flv", "mp3", "htc"};

     private static final String IGNORE_STATIC_FILE_SUFFIX = "ignoreStaticFileSuffix";
     ...
     private static final String PATH_REGEX_ALL = "/*";
     ...
     private void initExclusionPath(FilterConfig filterConfig) {
         this.exclusions = initPathListSeparateByComma(filterConfig, EXCLUSIONS);
     }
      ...
      * @return
      */
     protected List<String> initPathListSeparateByComma(FilterConfig filterConfig, String key){
     ...
     protected boolean isContainsUrl(final HttpServletRequest request, List<String> urls){
         ...
         for (String url : urls) {
             if (url.equals(PATH_REGEX_ALL)){
                 return true;
             }
             if (url.endsWith(PATH_REGEX_ALL)) {
     ...
     private void initStaticFileList(FilterConfig filterConfig){
         //先初始化默认后缀
         ignoreSuffix.addAll(Arrays.asList(DEFAULT_STATIC_FILE_SUFFIX));
         String staticFileList = this.getPropertyFromInitParams(filterConfig, IGNORE_STATIC_FILE_SUFFIX, StringUtils.EMPTY);
